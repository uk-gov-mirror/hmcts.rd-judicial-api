...
public class IdamOpenIdClient {
    ...
    public Map<String, String> createUser(String userRole) {
        ...
        return createUser(userRole, generateRandomEmail(), "jrd-test", "jrd-test");
    }
    ...
    public String getJrdAdminOpenIdToken() {
        if (isNull(jrdAdminToken)) {
            jrdAdminToken = getToken(ROLE_JRD_ADMIN);
        }
        return jrdAdminToken;
    }

    public String getJrdSystemUserOpenIdToken() {
        if (isNull(jrdSystemUserToken)) {
            jrdSystemUserToken = getToken(ROLE_JRD_SYSTEM_USER);
        }
        return jrdSystemUserToken;
    }

    public String getOpenIdTokenByRole(String role) {
        if (StringUtils.isNotEmpty(role)) {
            if (ROLE_JRD_ADMIN.equals(role)) {
                return getJrdAdminOpenIdToken();
            } else if (ROLE_JRD_SYSTEM_USER.equals(role)) {
                return getJrdSystemUserOpenIdToken();
            } else {
                return getToken(role);
            }
        } else {
            getToken(ROLE_JRD_ADMIN);
        }
        return null;
    }
